...
DRV_HANDLE WDRV_PIC32MZW_Open(const SYS_MODULE_INDEX index, const DRV_IO_INTENT intent)
{
    ...
    if (false == pDcpt->isOpen)
    {
        ...
        if (WDRV_PIC32MZW_SYS_IDX_0 == index)
        {
            ...
            {
                pDcpt->pCtrl->assocInfo[i].handle = DRV_HANDLE_INVALID;
                pDcpt->pCtrl->assocInfo[i].rssi   = 0;
            }
        }
        else
        {
    ...
    switch(wid)
    {
        case DRV_WIFI_WID_RSSI:
        {
            ...
            if (NULL != pCtrl->pfAssociationRSSICB)
            {
                ...
                pCtrl->pfAssociationRSSICB((DRV_HANDLE)pDcpt, (WDRV_PIC32MZW_ASSOC_HANDLE)&pCtrl->assocInfo[0], pCtrl->assocInfo[0].rssi);
        ...
        case DRV_WIFI_WID_ASSOC_STAT:
        {
            ...
            else if ((1 == *pData) && (WDRV_PIC32MZW_CONN_STATE_CONNECTED != pCtrl->connectedState))
            {
                ...
                if (false == pCtrl->isAP)
                {
                    ...
                    pCtrl->assocInfo[0].peerAddress.valid = false;
                    pCtrl->assocInfo[0].authType          = WDRV_PIC32MZW_AUTH_TYPE_DEFAULT;
                    pCtrl->assocInfo[0].rssi              = 0;

                    memset(&pCtrl->assocInfo[0].peerAddress.addr, 0, WDRV_PIC32MZW_MAC_ADDR_LEN);
                    ...
                    if (NULL != pCtrl->pfConnectNotifyCB)
                    {
                        ...
                        pCtrl->pfConnectNotifyCB((DRV_HANDLE)pDcpt, (WDRV_PIC32MZW_ASSOC_HANDLE)&pCtrl->assocInfo[0], WDRV_PIC32MZW_CONN_STATE_CONNECTED);
                    }
                }
            }
        ...
        case DRV_WIFI_WID_BSSID:
        {
            ...
            if (false == pCtrl->isAP)
            {
                memcpy(&pCtrl->assocInfo[0].peerAddress.addr, pData, WDRV_PIC32MZW_MAC_ADDR_LEN);
        ...
        case DRV_WIFI_WID_GET_SCAN_RESULTS:
        {
            ...
            if((NULL != pScanRes) && (NULL != pCtrl->pfBSSFindNotifyCB) && (0 == pScanRes->ofTotal))
            {
...
void DRV_PIC32MZW_PacketMemFree(void *pPktBuff)
{
    ...
    if ((pk0PktBuff >= (void*)&pic32mzwHighPriPktList[0].pkt) && (pk0PktBuff < (void*)&pic32mzwHighPriPktList[NUM_HIGH_PRI_PKTS-1].pkt))
    {
        int i;
