...
static void APP_TcpipStack_EventHandler(TCPIP_NET_HANDLE hNet, TCPIP_EVENT event, const void *fParam)
{
    ...
    {
        SYS_PRINT("APP: %s Unknown event event = %d\r\n", netName, event);
    }
}
...
void APP_TcpipDhcp_EventHandler(TCPIP_NET_HANDLE hNet, TCPIP_DHCP_EVENT_TYPE evType, const void* param)
{
    switch(evType)
    {
        ...
        {
            SYS_MESSAGE("Connection to the DHCP server established\r\n");
            break;
        }
        case DHCP_EVENT_CONN_LOST:
        {
            SYS_MESSAGE("Connection to the DHCP server lost\r\n");
            break;
        }
        default:
            break;
...
static bool APP_BSSFindNotifyCallback(DRV_HANDLE handle, uint8_t index, uint8_t ofTotal, WDRV_PIC32MZW_BSS_INFO *pBSSInfo)
{
    ...
    if(ofTotal == 0)
    {
        SYS_MESSAGE("No AP Found Rescan\r\n");
        return true;
    }
    ...
    {
        SYS_PRINT("#%02d\r\n", ofTotal);
        SYS_PRINT(">>#  RI  Sec  Recommend CH BSSID             SSID\r\n");
        SYS_PRINT(">>#      Cap  Auth Type\r\n>>#\r\n");
    }
    
    if (WDRV_PIC32MZW_STATUS_OK == WDRV_PIC32MZW_BSSFindGetInfo(appData.wdrvHandle, &bssInfo))
    {
        SYS_PRINT(">>%02d %d 0x%02x ", index, bssInfo.rssi, bssInfo.secCapabilities);
        ...
        switch (bssInfo.authTypeRecommended)
        {
            ...
            default:
            {
                SYS_PRINT("Not Avail", 9);
                break;
            }
        }

        SYS_PRINT(" %02d %02X:%02X:%02X:%02X:%02X:%02X %s\r\n", bssInfo.ctx.channel,
...
void APP_Scan(uint8_t channel, SCAN_TYPE scanType, uint16_t scanTime)
{
    ...
    {
        SYS_MESSAGE("APP: Invalid channel \r\n");
        return;
    }
    
    SYS_PRINT("APP_scan: channel: %d scanType: %d\r\n",channel, scanType);
    
    if(scanType == ACTIVE)
    {
        if (WDRV_PIC32MZW_STATUS_OK != WDRV_PIC32MZW_BSSFindSetScanParameters(appData.wdrvHandle, scanTime, 0))
        {
            SYS_MESSAGE("APP: Error updating Scan Parameters\r\n");
            return;
        }
    }
    else
...
void APP_RSSIGet()
{
    ...
    {
        SYS_MESSAGE("Device not connected\r\n");
        return;
    }
...
static bool APP_WIFI_Config() 
{
    ...
    switch (authMode) 
    {
        case OPEN:
        {
            ...
            {
                SYS_MESSAGE("APP: Unable to set Authentication\r\n");
                ret = false;
            }
            break;
        }
        case WPA2:
        {
            ...
            {
                SYS_MESSAGE("APP: Unable to set authentication to WPA2\r\n");
                ret = false;
            }
            break;
        }
        case WPAWPA2MIXED:
        {
            ...
            {
                SYS_MESSAGE("APP: Unable to set authentication to WPAWPA2 MIXED\r\n");
                ret = false;
            }
            break;
        }
        ...
        case WPA3:
        {
            ...
            {
                SYS_MESSAGE("APP: Unable to set authentication to WPA3 \r\n");
                ret = false;
            }
            break;
        }
        case WPA2WPA3MIXED:
        {
            ...
            {
                SYS_MESSAGE("APP: Unable to set authentication to WPA2WPA3 MIXED \r\n");
                ret = false;
            }
            break;
        }
        ...
        case WEP:
        {
            ...
            {
                SYS_MESSAGE("APP: Unable to set authentication to WEP \r\n");
                ret = false;
            }
            if (WDRV_PIC32MZW_STATUS_OK != WDRV_PIC32MZW_AuthCtxSharedKey(&g_wifiConfig.authCtx, true))
            {
                SYS_MESSAGE("APP: Unable to Enable shared key authentication \r\n");
                ret = false;
            }
            break;
        }
...
void APP_Initialize ( void )
{
    ...
    SYS_MESSAGE("APP: Initialization Successful\r\n");
}
...
//    TCPIP_MAC_ADDR macAddr;
    ...
    switch ( appData.state )
    {
        ...
        case APP_TCPIP_WAIT_FOR_TCPIP_INIT:
        {
            ...
            if (tcpipStat < 0) 
            {
                SYS_MESSAGE( "TCP/IP stack initialization failed!\r\n" );
        ...
        case APP_WIFI_CONFIG:
        {
            if(app_controlData.wlanConfigValid) 
            {
                ...
                {
                    SYS_MESSAGE("APP: Failed connecting to Wi-Fi\r\n" );
